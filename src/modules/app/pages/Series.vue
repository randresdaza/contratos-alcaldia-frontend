<template>
  <div class="q-pa-md">
    ...
    <q-table class="my-sticky-header-table-series q-mt-xs" virtual-scroll flat bordered title="Series" :rows="data"
      :columns="columns" :virtual-scroll-sticky-size-start="48" row-key="nombre" v-model:pagination.sync="pagination"
...
export default {
  setup() {
    ...
    const getData = async (filter) => {
      await api.get(`/series/?filter=${filter}&page=${pagination.value.page}&page_size=${pagination.value.rowsPerPage}`)
        .then(result => {
      ...
      await api.post('/series/', formData.value)
        ...
        .catch(e => {
          if (e.response.status == 400) {
            Swal.fire(
              {
                html: `Ya existe la serie con el nombre (${formData.value.nombre}).`,
                icon: 'info'
              }
            )
      ...
      await api.put(`/series/${editData.value.id}/`, editData.value)
        ...
        .catch(e => {
          ...
          if (e.response.status == 400) {
            Swal.fire(
              {
                html: `Ya existe la serie con el nombre (${editData.value.nombre}).`,
                icon: 'info'
              }
            )
          } else if (e.response.status == 401) {
                ...
                Swal.fire(
                  {
                    icon: 'error',
                    title: 'Error',
                    html: `La serie (${data.nombre}) se encuentra asociada a un contrato.`,
                  }
                )
              }
            })
