<template>
  <div class="q-pa-md">
    <q-form @submit="addData">
      <div class="row items-center">
        <q-input class="q-mb-md q-pt-md q-pb-md" style="width: 300px;max-width: 80vw; max-height: 80vw;" filled
          v-model="formData.nombre" label="Nombre de la Dependencia" type="text" lazy-rules
    ...
    <q-table class="my-sticky-virtscroll-table q-mt-xs col justify-center" virtual-scroll flat bordered
      title="Dependencias" :rows="sortedData" :columns="columns" :virtual-scroll-sticky-size-start="48" row-key="nombre"
...
export default {
  setup() {
      ...
      await api.post('/dependencias/', formData.value)
        ...
        .catch(e => {
          if (e.response.status == 400) {
            Swal.fire(
              {
                html: `Ya existe la dependencia ${formData.value.nombre}.`,
                icon: 'info'
              }
            )
      ...
      await api.put(`/dependencias/${editData.value.id}/`, editData.value)
        ...
        .catch(e => {
          ...
          if (e.response.status == 400) {
            Swal.fire(
              {
                html: `Ya existe la dependencia ${editData.value.nombre}.`,
                icon: 'info'
              }
            )
          } else if (e.response.status == 401) {
    ...
    const toUpperCase = () => {
      ...
      editData.value.nombre = editData.value.nombre.toUpperCase()
    }
