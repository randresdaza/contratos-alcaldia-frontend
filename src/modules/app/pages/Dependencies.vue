<template>
  <div class="q-pa-md">
    <q-form @submit="addData">
      <div class="row items-center">
        <q-input style="width: 300px;max-width: 80vw; max-height: 80vw;" filled v-model="formData.nombre"
          label="Nombre de la Dependencia" type="text" lazy-rules
    ...
    <q-table class="my-sticky-header-table-dependencies q-mt-xs" virtual-scroll flat bordered title="Dependencias"
      :rows="data" :columns="columns" :virtual-scroll-sticky-size-start="48" row-key="nombre"
    ...
    <q-dialog v-model="modal">
      <q-card class="q-gutter-xs" style="width: 500px;max-width: 80vw; max-height: 80vw;">
        <q-card-section class="row justify-center q-pa-xs q-mt-lg">
          <div class="text-h5">Información de la Dependencia</div>
        </q-card-section>
        <q-card-section>
...
export default {
  setup() {
    ...
    const getData = async (filter) => {
      await api.get(`/dependencias/?filter=${filter}&page=${pagination.value.page}&page_size=${pagination.value.rowsPerPage}`)
        .then(result => {
    ...
    const addData = async () => {
      await api.post('/dependencias/', formData.value)
        .then(result => {
        ...
        .catch(async (e) => {
          if (e.response.status == 400) {
            Swal.fire(
              {
                html: `Ya existe la dependencia con el nombre (${formData.value.nombre}).`,
                icon: 'info'
              }
            )
    ...
    const updateData = async () => {
      await api.put(`/dependencias/${editData.value.id}/`, editData.value)
        .then(result => {
          getData(filter.value)
          modal.value = false
        ...
        .catch(async (e) => {
          ...
          if (e.response.status == 400) {
            Swal.fire(
              {
                html: `Ya existe la dependencia con el nombre (${editData.value.nombre}).`,
                icon: 'info'
              }
            )
          } else if (e.response.status == 401) {
    ...
    const onDelete = (data) => {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          api.delete(`/dependencias/${data.id}/`)
            .then(result => {
              getData(filter.value)
              Swal.fire(
                'Eliminado!',
                ...
                Swal.fire(
                  {
                    icon: 'error',
                    title: 'Error',
                    html: `La dependencia (${data.nombre}) se encuentra asociada a un contrato.`,
                  }
                )
              }
            })
...
.my-sticky-header-table-dependencies
